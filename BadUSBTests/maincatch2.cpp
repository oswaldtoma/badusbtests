...
TEST_CASE("TestProject1", "TestSingleChar")
{
	char buffer = '\0';
...
TEST_CASE("TestProject2", "TestKeyCombination")
{
	char buffer[30] = { '\0' };
...
TEST_CASE("TestProject3", "TestWordWithQuotationMarks")
{
	char buffer[20] = { '\0' };
	ScriptLang::getLineBytesArray("\"Samochod123\"", buffer);

	EXPECT_STREQ((char*)"\"Samochod123\"", buffer);
}

TEST_CASE("TestProject4", "TestWordWithQuotationMarksAndSomethingAfter")
{
	char buffer[20] = { '\0' };
	ScriptLang::getLineBytesArray("\"Samochod123\" r", buffer); //should abandon and return empty array

	EXPECT_STREQ((char*)"", buffer);
}

TEST_CASE("TestProject5", "TestWordWithQuotationMarksAndSpaces")
{
	char buffer[30] = { '\0' };
	ScriptLang::getLineBytesArray("\"Samochod123 Ala ma kota\"", buffer); //should return bytes including spaces 

	EXPECT_STREQ((char*)"\"Samochod123 Ala ma kota\"", buffer);
}

TEST_CASE("TestProject6", "TestWordWithSpecialKeyPhraseInside")
{
	char buffer[30] = { '\0' };
	ScriptLang::getLineBytesArray("\"Samochod123 OS\"", buffer); //should return bytes including spaces, NO SPECIAL KEY

	EXPECT_STREQ((char*)"\"Samochod123 OS\"", buffer);
}

TEST_CASE("TestProject7", "TestComment")
{
	char buffer[30] = { '\0' };
...
//SCRIPT MANAGER
TEST_CASE("TestProject8", "ScriptManagerRowCounter")
{
...
TEST_CASE("TestProject9", "ScriptManagerTest1")
{
	...
	EXPECT_STREQ((const char*)rows[0].rowArray, "OS R");

	string = "OS R\n\"Samochod123\"";
	ScriptManager::getRows((uint8_t*)string, 18, rows, 20);
	EXPECT_STREQ((const char*)rows[0].rowArray, "OS R");
	EXPECT_STREQ((const char*)rows[1].rowArray, "\"Samochod123\"");
...
TEST_CASE("TestProject10", "ScriptManagerWithKeyboard")
{
...
TEST_CASE("TestProject11", "ScriptManagerTest2")
{
	KeyboardFake keyboard;
	ScriptManager::init(&keyboard);
...
TEST_CASE("TestProject12", "ScriptManagerTest3")
{
	KeyboardFake keyboard;
	ScriptManager::init(&keyboard);
...
TEST_CASE("TestProject13", "ScriptManagerTest4LongText")
{
	KeyboardFake keyboard;
	ScriptManager::init(&keyboard);
...
TEST_CASE("TestProject14", "ScriptManagerKeysOneAfterAnother")
{
	KeyboardFake keyboard;
	ScriptManager::init(&keyboard);

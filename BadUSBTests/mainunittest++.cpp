...
TEST(TestWordWithQuotationMarks)
{
	char buffer[20] = { '\0' };
...
TEST(TestWordWithQuotationMarksAndSomethingAfter)
{
	char buffer[20] = { '\0' };
...
TEST(TestWordWithQuotationMarksAndSpaces)
{
	char buffer[30] = { '\0' };
...
TEST(TestWordWithSpecialKeyPhraseInside)
{
	char buffer[30] = { '\0' };
...
TEST(ScriptManagerWithKeyboard)
{
...
TEST(ScriptManagerTest4LongText)
{
	KeyboardFake keyboard;
	ScriptManager::init(&keyboard);
...
TEST(ScriptManagerKeysOneAfterAnother)
{
	KeyboardFake keyboard;
	ScriptManager::init(&keyboard);
